...
class PCTransformer(nn.Module):
    def __init__(self, config):
        ...
        #   global_coarse_pred: 256 absolute coords from global_feature (hole-filling)
        #   coarse_pred:        256 per-token offsets from coor (surface alignment)
        # Total: 256 + 256 = 512 = num_query
        ...
        self.global_coarse_pred = nn.Sequential(
            ...
            nn.GELU(),
            nn.Linear(1024, self.num_global_points * 3)  # absolute coords
        )
    ...
    def forward(self, xyz):
        ...
        global_feature = torch.max(token_features, dim=1)[0] # (B, global_feature_dim) — for mlp_query only
        ...
        # Global: absolute coordinates from global feature (hole-filling)
        coarse_global = self.global_coarse_pred(global_feature).reshape(B, self.num_global_points, 3)  # (B, num_global_points, 3)
        # Concatenate: local + global = num_query total
        coarse = torch.cat([coarse_local, coarse_global], dim=1)  # (B, num_query, 3)
        
        # Local Feature Induced Query: associate each coarse point with its feature
        coarse_features = torch.cat([
            token_features,
            global_feature.unsqueeze(1).expand(-1, self.num_global_points, -1)
        ...
        coarse_features = torch.gather(coarse_features, 1, idx[:,:self.num_query].expand(-1, -1, coarse_features.size(-1)))
        ...
        if self.training:
            ...
            return q, coarse, denoise_length, (coor, fm_cond, coarse_local, coarse_global)
...
class AdaPoinTr_PGST(nn.Module):
    ...
    def get_loss(self, ret, gt, epoch=1):
        ...
        # recon loss (original, kept as-is for backward compatibility)
        gt_coarse = misc.fps(gt, pred_coarse.size(1))
        loss_coarse = self.loss_func(pred_coarse, gt_coarse)
        loss_fine = self.loss_func(pred_fine, gt)

        # Flow Matching loss: explicit point-to-point velocity supervision
        coor, fm_cond, coarse_local, coarse_global = fm_data
        B, G, _ = coor.shape

        with torch.no_grad():
            gt_local = misc.fps(gt, G)            # (B, 384, 3) uniform targets
            cost = torch.cdist(coor, gt_local)    # (B, 384, 384)
            assign = sinkhorn_matching(cost)       # (B, 384) one-to-one OT
            x_1 = torch.gather(gt_local, 1, assign.unsqueeze(-1).expand(-1, -1, 3))
        ...
        # === NEW: Global coverage loss ===
        # Give the 128 global points a dedicated FPS target (1:1 strong signal)
        N_global = coarse_global.size(1)
        gt_global = misc.fps(gt, N_global)        # (B, 128, 3) uniform GT targets
        loss_global = self.loss_func(coarse_global, gt_global)

        # === NEW: Repulsion among 128 global points ===
        # Penalize global points that are too close to each other
        dists_gg = torch.cdist(coarse_global, coarse_global)   # (B, 128, 128)
        eye_mask = torch.eye(N_global, device=dists_gg.device).unsqueeze(0)
        dists_gg = dists_gg + eye_mask * 1e6                   # exclude self
        min_nn_dist = dists_gg.min(dim=-1)[0]                  # (B, 128)
        repulsion_margin = 0.05
        loss_repulsion = torch.mean(torch.clamp(repulsion_margin - min_nn_dist, min=0))

        w_global = 3.0
        w_repulsion = 1.0
        loss_recon = (loss_coarse + loss_fine + loss_fm
