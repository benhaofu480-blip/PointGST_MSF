...
class PCSA(nn.Module):
    def __init__(self, dim, adapt_dim, use_sama=False, use_mssa=False):
        super().__init__()
        self.device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
        self.adapt_dim = adapt_dim
        self.use_sama = use_sama
        ...
        if self.use_mssa:
            self.adapt1 = nn.Linear(adapt_dim, adapt_dim)
            nn.init.zeros_(self.adapt1.weight)
            nn.init.zeros_(self.adapt1.bias)
            self.adapt2 = nn.Linear(adapt_dim, adapt_dim)
            nn.init.zeros_(self.adapt2.weight)
            nn.init.zeros_(self.adapt2.bias)
            # Adaptive Scale Gate: 学习每个点应信任 16x16 还是 32x32 尺度
            # 初始时 sigmoid(0)=0.5，两个尺度等权，等价于 baseline
            self.scale_gate = nn.Sequential(
                nn.Linear(adapt_dim, adapt_dim // 2),
                nn.GELU(),
                nn.Linear(adapt_dim // 2, 1),
            )
        ...
        # 对频域中不同频率分量估计可靠性，抑制不可靠频率的 adapt
        if self.use_sama:
            mid_dim = max(adapt_dim // 2, 16)
            self.energy_proj = nn.Linear(1, mid_dim)
            self.gate_net = nn.Sequential(
                nn.Linear(mid_dim, mid_dim),
                nn.GELU(),
                nn.Linear(mid_dim, adapt_dim),
                nn.Sigmoid()
            )
            # 零初始化最后一层，使初始时 SAMA 不影响 adapt
            nn.init.zeros_(self.gate_net[-2].weight)
            nn.init.zeros_(self.gate_net[-2].bias)

    def _apply_adapt(self, x_f, adapt_layer):
        return adapt_layer(x_f)

    def _apply_sama(self, x_f, energy_proj, gate_net):
        """SAMA: 用频谱能量估计频率可靠性，门控 adapt 输出。
        x_f: (B, group_num, group_size, adapt_dim)
        """
        # 频谱能量: 每个频率分量在特征维度上的平均能量
        spectral_energy = (x_f ** 2).mean(dim=-1, keepdim=True)  # (B, G, S, 1)
        energy_feat = energy_proj(spectral_energy)  # (B, G, S, mid_dim)
        gate = gate_net(energy_feat)  # (B, G, S, adapt_dim)  值域 [0, 1]
        return gate
    ...
    def forward(self, input, sub_U, idx):
        ...
        B0, group_num0, group_size0, _ = sub_U[0].shape  # B, G//16, 16, 16
        G0 = group_num0 * group_size0  # G
        sub_x0 = sort(x, idx[0])
        sub_x0 = sub_x0.reshape(B0, group_num0, group_size0, self.adapt_dim)

        B1, group_num1, group_size1, _ = sub_U[1].shape  # B, G//32, 32, 32
        G1 = group_num1 * group_size1  # G
        sub_x1 = sort(x, idx[0])
        sub_x1 = sub_x1.reshape(B1, group_num1, group_size1, self.adapt_dim)

        # Scale 1 (sub_U0): group_size=16
        sub_x_f0 = sub_U[0].transpose(-2, -1) @ sub_x0
        sub_h_f0 = sub_x_f0
        sub_x_f0 = self.norm_ly2(sub_x_f0)
        if self.use_mssa:
            adapt_out = self._apply_adapt(sub_x_f0, self.adapt1)
        else:
            adapt_out = self._apply_adapt(sub_x_f0, self.adapt)
        if self.use_sama:
            gate0 = self._apply_sama(sub_x_f0, self.energy_proj, self.gate_net)
            adapt_out = adapt_out * gate0
        sub_x_f0 = sub_h_f0 + self.drop_adapt1(self.act(self.drop_out(adapt_out)))
        sub_x0 = sub_U[0] @ sub_x_f0

        sub_x0 = sub_x0.reshape(B0, G0, self.adapt_dim)
        sub_x0 = sort(sub_x0, idx[1])

        # Scale 2 (sub_U1): group_size=32
        sub_x_f1 = sub_U[1].transpose(-2, -1) @ sub_x1
        sub_h_f1 = sub_x_f1
        sub_x_f1 = self.norm_ly2(sub_x_f1)
        if self.use_mssa:
            adapt_out = self._apply_adapt(sub_x_f1, self.adapt2)
        else:
            adapt_out = self._apply_adapt(sub_x_f1, self.adapt)
        if self.use_sama:
            gate1 = self._apply_sama(sub_x_f1, self.energy_proj, self.gate_net)
            adapt_out = adapt_out * gate1
        sub_x_f1 = sub_h_f1 + self.drop_adapt2(self.act(self.drop_out(adapt_out)))
        sub_x1 = sub_U[1] @ sub_x_f1

        sub_x1 = sub_x1.reshape(B1, G1, self.adapt_dim)
        sub_x1 = sort(sub_x1, idx[1])

        # Adaptive Scale Gate: 根据特征决定信任哪个尺度
        # sub_x0, sub_x1 都已 reshape 回 (B, G, adapt_dim)
        if self.use_mssa:
            alpha = torch.sigmoid(self.scale_gate((sub_x0 + sub_x1) / 2))  # (B, G, 1)
            x = alpha * sub_x0 + (1 - alpha) * sub_x1
        else:
            x = sub_x0 + sub_x1
...
class SelfAttnBlockApi(nn.Module):
    ...
    def __init__(
            ...
            k=10, n_group=2, use_sama=False, use_mssa=False
        ):
        ...
        self.gft_adapter=PCSA(dim, 36, use_sama=use_sama, use_mssa=use_mssa)
    ...
    def __init__(self, embed_dim=256, depth=4, num_heads=4, mlp_ratio=4., qkv_bias=False, init_values=None,
        ...
        for i in range(depth):
            self.blocks.append(SelfAttnBlockApi(
                ...
                use_sama=use_sama, use_mssa=use_mssa
            ))
...
class FlowMatchingOffset(nn.Module):
    """
    Conditional OT-Flow Matching for local coarse offset generation.
    Learns velocity field v(x_t, t | cond) that transports grouper centers
    toward matched GT surface positions via straight-line ODE trajectories.
    """
    def __init__(self, cond_dim, hidden_dim=512, time_dim=128):
        super().__init__()
        ...
        self.vel_net = nn.Sequential(
            nn.Linear(3 + time_dim + cond_dim, hidden_dim),
            nn.SiLU(),
    ...
    def forward(self, x_t, t, cond):
        """
        Args:
            x_t:  (B, N, 3)  positions at flow time t
            t:    (B,)        time in [0, 1]
            cond: (B, N, D)   conditioning (token_features ++ coor)
        Returns: velocity (B, N, 3)
        """
        B, N, _ = x_t.shape
        t_emb = self.time_mlp(t.view(B, 1))            # (B, time_dim)
        t_emb = t_emb.unsqueeze(1).expand(-1, N, -1)    # (B, N, time_dim)
        return self.vel_net(torch.cat([x_t, t_emb, cond], dim=-1))

    @torch.no_grad()
    def sample(self, x_0, cond, num_steps=10):
...
def sinkhorn_matching(cost, iters=20, reg=0.05):
    """
    GPU-friendly Sinkhorn OT matching.
    Returns hard assignment: (B, N) indices into M targets.
    """
    log_K = -cost / reg
    for _ in range(iters):
        log_K = log_K - torch.logsumexp(log_K, dim=2, keepdim=True)
        log_K = log_K - torch.logsumexp(log_K, dim=1, keepdim=True)
    return log_K.argmax(dim=2)
...
class PCTransformer(nn.Module):
    def __init__(self, config):
        ...
        self.use_sama = getattr(config, 'use_sama', False)
        self.use_mssa = getattr(config, 'use_mssa', False)
        ...
        encoder_config.use_sama = self.use_sama
        encoder_config.use_mssa = self.use_mssa
        ...
        # Global-Local Anchor coarse generation:
        #   global_coarse_pred: absolute coordinate generation from global features (hole-filling)
        #   flow_offset:        per-token offsets from all grouper centers (surface alignment)
        # Total: num_global_points + center_num[-1] = num_query
        self.num_global_points = self.num_query - self.center_num[-1]
        
        # Absolute coordinate generation MLP: input max+mean pooled features (2048-d)
        self.global_coarse_pred = nn.Sequential(
            nn.Linear(global_feature_dim * 2, 1024),
            nn.GELU(),
            nn.Linear(1024, 1024),
            nn.GELU(),
            nn.Linear(1024, self.num_global_points * 3)
        )
        # Learnable positional encoding to break feature homogenization for 128 global tokens
        self.global_token_pe = nn.Parameter(torch.zeros(1, self.num_global_points, global_feature_dim))
        trunc_normal_(self.global_token_pe, std=.02)
        self.flow_offset = FlowMatchingOffset(
            cond_dim=global_feature_dim + 3,   # token_features ++ coor
            hidden_dim=512,
            time_dim=128
        )
    ...
    def forward(self, xyz):
        ...
        x = self.encoder(x + pe, coor, [sub_U0, sub_U1], [idx0,idx1]) # (B, G, embed_dim)
        token_features = self.increase_dim(x) # (B, G, global_feature_dim) — keep all tokens
        # Dual-pooling for global coordinate generation
        max_feat = torch.max(token_features, dim=1)[0]   # (B, 1024) — also fed to frozen downstream
        mean_feat = torch.mean(token_features, dim=1)     # (B, 1024)
        global_feature = max_feat  # keep pure max-pool for frozen downstream compatibility

        # Global-Local Anchor coarse generation
        # Local: flow matching offset from grouper centers (surface alignment)
        fm_cond = torch.cat([token_features, coor], dim=-1)  # (B, G, 1027)
        if self.training:
            t_zero = torch.zeros(B, device=coor.device, dtype=coor.dtype)
            coarse_offset = self.flow_offset(coor, t_zero, fm_cond)
            coarse_local = coor + coarse_offset
        else:
            coarse_local = self.flow_offset.sample(coor, fm_cond, num_steps=10)

        # Global: absolute coordinate generation from dual-pooled features (hole-filling)
        generator_input = torch.cat([max_feat, mean_feat], dim=-1)  # (B, 2048)
        coarse_global = self.global_coarse_pred(generator_input).reshape(B, self.num_global_points, 3)

        # Concatenate: local + global = num_query total
        coarse = torch.cat([coarse_local, coarse_global], dim=1)  # (B, num_query, 3)
        
        # Per-token Feature Induced Query: local tokens have per-token features,
        # global tokens use broadcast global_feature + learnable positional encoding
        global_features_with_pe = global_feature.unsqueeze(1).expand(-1, self.num_global_points, -1) + self.global_token_pe
        coarse_features = torch.cat([
            token_features,
            global_features_with_pe
        ], dim=1)  # (B, num_query, 1024)

        mem = self.mem_link(x)

        # query_ranking: keep forward identical to pretrained; pass idx to loss for unshuffle
        ...
        coarse_features = torch.gather(
            coarse_features, 1, idx[:, :self.num_query].expand(-1, -1, coarse_features.size(-1))
        )
        ...
        if self.training:
            ...
            # For denoise points, use global feature
            denoise_features = global_feature.unsqueeze(1).expand(-1, denoise_length, -1)
            coarse_features = torch.cat([coarse_features, denoise_features], dim=1)
            ...
            # Ranking index for first num_query points only (idx is (B, num_query, 1), not 768)
            sort_idx = idx[:, : self.num_query, :].contiguous()
            return q, coarse, denoise_length, (coor, fm_cond, coarse_local, coarse_global, sort_idx)
...
class AdaPoinTr_PGST(nn.Module):
    ...
    def get_loss(self, ret, gt, epoch=1):
        pred_coarse, denoised_coarse, denoised_fine, pred_fine, fm_data = ret
        coor, fm_cond, coarse_local, coarse_global, sort_idx = fm_data

        assert pred_coarse.size(1) == self.num_query
        assert pred_fine.size(1) == gt.size(1)

        idx_denoise = knn_point(self.factor, gt, denoised_coarse)
        denoised_target = index_points(gt, idx_denoise).reshape(gt.size(0), -1, 3)
        assert denoised_target.size(1) == denoised_fine.size(1)
        loss_denoised = self.loss_func(denoised_fine, denoised_target) * 0.5

        # Decoupled coarse: only supervise 384 local points with loss_base,
        # let 128 global points be exclusively supervised by loss_global_hole
        gt_local_fps = misc.fps(gt, coarse_local.size(1))  # 384
        loss_base = self.loss_func(coarse_local, gt_local_fps)

        # Global loss: Dynamic Hole Targeting + Sinkhorn absolute coordinate supervision
        B_g, N_g, _ = coarse_global.shape  # N_g = 128
        with torch.no_grad():
            gt_candidate_pool = misc.fps(gt, 1024)  # (B, 1024, 3)
            dist_gt_local = torch.cdist(gt_candidate_pool, coor)  # (B, 1024, 384)
            min_dist, _ = dist_gt_local.min(dim=-1)  # (B, 1024)
            _, hole_idx = torch.topk(min_dist, k=N_g, dim=-1)  # (B, 128)
            hole_targets = torch.gather(
                gt_candidate_pool, 1, hole_idx.unsqueeze(-1).expand(-1, -1, 3)
            )  # (B, 128, 3)
            # Sinkhorn 1-to-1 matching
            cost = torch.cdist(coarse_global, hole_targets)
            assign = sinkhorn_matching(cost)
            matched_hole_targets = torch.gather(
                hole_targets, 1, assign.unsqueeze(-1).expand(-1, -1, 3)
            )

        loss_global_hole = torch.nn.functional.mse_loss(coarse_global, matched_hole_targets)

        # Flow matching loss (local branch, unchanged)
        B, G, _ = coor.shape
        with torch.no_grad():
            gt_local_fps = misc.fps(gt, G)
            cost_local = torch.cdist(coor, gt_local_fps)
            assign_local = sinkhorn_matching(cost_local)
            x_1 = torch.gather(gt_local_fps, 1, assign_local.unsqueeze(-1).expand(-1, -1, 3))

        t = torch.rand(B, device=coor.device, dtype=coor.dtype)
        t_exp = t.view(B, 1, 1)
        x_t = (1.0 - t_exp) * coor + t_exp * x_1
        v_target = x_1 - coor
        v_pred = self.base_model.flow_offset(x_t, t, fm_cond)
        ...
        loss_recon = (
            loss_fine
            + loss_base
            + 1.0 * loss_global_hole
        )

        return loss_denoised, loss_recon + loss_fm

    def forward(self, xyz):
        q, coarse_point_cloud, denoise_length, fm_data = self.base_model(xyz)
        ...
        if self.training:
            ...
            ret = (pred_coarse, denoised_coarse, denoised_fine, pred_fine, fm_data)
            return ret
