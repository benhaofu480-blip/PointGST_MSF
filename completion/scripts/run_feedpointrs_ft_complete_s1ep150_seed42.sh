#!/usr/bin/env bash
# Stage-2 FeedPoinTrS-FT (crop [0.2,0.4]) on PCN full train, init from Stage-1 complete ep150 ckpt-best.
# Usage: bash scripts/run_feedpointrs_ft_complete_s1ep150_seed42.sh
#   MSF_SEED=42 MSF_DDP_PORT=29516 MSF_DDP_GPUS=0,1 bash scripts/run_feedpointrs_ft_complete_s1ep150_seed42.sh
...
SEED="${MSF_SEED:-42}"
EXP="exp_MSF_Pure_Group_sigmoid_feedpointrs_ft_crop02_04_complete_s1ep150_seed${SEED}"
PORT="${MSF_DDP_PORT:-29516}"
LOG_DIR="logs/complete"
EXP_DIR="experiments/AdaPoinTr_MSF_Pure_Group_sigmoid_feedpointrs_ft_complete/PCN_models/${EXP}"
TRAIN_LOG="${LOG_DIR}/train_feedpointrs_s1ep150_seed${SEED}.log"

S1_CKPT_NFS="experiments/AdaPoinTr_MSF_Pure_Group_sigmoid_complete/PCN_models/exp_MSF_Pure_Group_sigmoid_complete_seed${SEED}/ckpt-best.pth"
S1_CKPT_TMP="/tmp/sigmoid_complete_s1ep150_ckpt-best.pth"
...
if [[ ! -f "$S1_CKPT_NFS" ]]; then
  echo "ERROR: Stage-1 ep150 ckpt-best not found: $S1_CKPT_NFS" >&2
  exit 1
fi
...
echo "Stage-2 FeedPoinTrS-FT (init Stage-1 ep150 ckpt-best)."
echo "  CONFIG=$CONFIG"
echo "  EXP=$EXP"
