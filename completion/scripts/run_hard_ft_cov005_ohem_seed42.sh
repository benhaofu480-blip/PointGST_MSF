#!/usr/bin/env bash
# Stage-2: cover λ=0.05 + batch OHEM, full PCN val (800) for ckpt selection, seed=42.
set -euo pipefail
cd "$(dirname "$0")/.."
...
CFG="cfgs/PCN_models/AdaPoinTr_MSF_Pure_Group_sigmoid_hard_ft_cov005_ohem.yaml"
EXP="exp_MSF_Pure_Group_sigmoid_hard_ft_cov005_ohem_seed42"
LOG="${MSF_LOG:-/tmp/msf_hard_ft_cov005_ohem_seed42.log}"

for f in data/PCN_hard/hard_train_ft_mix.txt "$CKPT"; do
  [[ -f "$f" ]] || { echo "Missing: $f" >&2; exit 1; }
done

echo "GPU=$CUDA_VISIBLE_DEVICES  seed=42  val=full(800)  OHEM=on  init=$CKPT  exp=$EXP"
echo "log=$LOG"
