#!/usr/bin/env bash
# Official PCN test for Exp2 single-pass 100ep (early-stop @30): ckpt-best + ckpt-last.
set -euo pipefail
...
EXP_DIR="${ROOT}/experiments/AdaPoinTr_MSF_Pure_Group_sigmoid_stage2_singlepass_100ep/PCN_models/exp_stage2_exp2_singlepass_100ep_seed42"
CFG="cfgs/PCN_models/AdaPoinTr_MSF_Pure_Group_sigmoid_stage2_singlepass_100ep.yaml"
LOG_BEST="${LOG_ROOT}/test_stage2_exp2_singlepass_100ep_best.log"
LOG_LAST="${LOG_ROOT}/test_stage2_exp2_singlepass_100ep_last.log"
WATCH="${LOG_ROOT}/watch_stage2_exp2_singlepass_100ep_tests.log"
...
if pgrep -f "main.py --test.*exp2_singlepass_100ep" >/dev/null 2>&1; then
  echo "ERROR: Exp2 test already running." >&2
  exit 1
fi
...
exp = Path("experiments/AdaPoinTr_MSF_Pure_Group_sigmoid_stage2_singlepass_100ep/PCN_models/exp_stage2_exp2_singlepass_100ep_seed42")
...
CUDA_VISIBLE_DEVICES="$GPU_BEST" nohup "$PYTHON" -u main.py --test \
  ...
  --config "$CFG" \
  --exp_name test_stage2_exp2_singlepass_100ep_best \
  --model pgst \
  --num_workers 4 \
  >>"$LOG_BEST" 2>&1 &
BEST_PID=$!
...
(
  ...
  CUDA_VISIBLE_DEVICES="$GPU_LAST" nohup "$PYTHON" -u main.py --test \
    ...
    --config "$CFG" \
    --exp_name test_stage2_exp2_singlepass_100ep_last \
    --model pgst \
    --num_workers 4 \
    >>"$LOG_LAST" 2>&1 &
