#!/usr/bin/env bash
# Stage-2 error-aware feedback crop, dual-GPU DDP (2 processes, one GPU each).
set -euo pipefail
...
LOG="${LOG_ROOT}/stage2_error_crop_ddp_seed${SEED}.log"
...
if pgrep -f "main.py.*error_crop" >/dev/null 2>&1; then
  ...
  pgrep -af "main.py.*error_crop" || true
  exit 1
fi
...
echo "exp=$EXP"
echo "log=$LOG"
echo "Monitor: grep -E 'Distributed Data parallel|crop_mode|Validation|Early Stop' $LOG"
