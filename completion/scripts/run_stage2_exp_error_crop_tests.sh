#!/usr/bin/env bash
# Official PCN test for Stage-2 error_crop (seed42): ckpt-best then ckpt-last.
set -euo pipefail
...
EXP_DIR="${ROOT}/experiments/AdaPoinTr_MSF_Pure_Group_sigmoid_feedpointrs_ft_error_crop/PCN_models/exp_stage2_error_crop_seed42"
CFG="cfgs/PCN_models/AdaPoinTr_MSF_Pure_Group_sigmoid_feedpointrs_ft_error_crop.yaml"
LOG_BEST="${LOG_ROOT}/test_stage2_error_crop_best.log"
LOG_LAST="${LOG_ROOT}/test_stage2_error_crop_last.log"
WATCH="${LOG_ROOT}/watch_stage2_error_crop_tests.log"
...
if pgrep -f "main.py --test.*error_crop" >/dev/null 2>&1; then
  echo "ERROR: error_crop test already running." >&2
  exit 1
fi
...
exp = Path("experiments/AdaPoinTr_MSF_Pure_Group_sigmoid_feedpointrs_ft_error_crop/PCN_models/exp_stage2_error_crop_seed42")
...
CUDA_VISIBLE_DEVICES="$GPU_BEST" nohup "$PYTHON" -u main.py --test \
  ...
  --config "$CFG" \
  --exp_name test_stage2_error_crop_best \
  --model pgst \
  --num_workers 4 \
  >>"$LOG_BEST" 2>&1 &
BEST_PID=$!
...
(
  ...
  CUDA_VISIBLE_DEVICES="$GPU_LAST" nohup "$PYTHON" -u main.py --test \
    ...
    --config "$CFG" \
    --exp_name test_stage2_error_crop_last \
    --model pgst \
    --num_workers 4 \
    >>"$LOG_LAST" 2>&1 &
...
echo "watch running in background; monitor:"
