#!/usr/bin/env bash
# Official PCN test for lr=1e-4 ablation (Exp1-ON crop 0.2-0.4).
# If ckpt-best and ckpt-last are the same epoch, only test once.
set -euo pipefail
...
EXP_DIR="${ROOT}/experiments/AdaPoinTr_MSF_Pure_Group_sigmoid_feedpointrs_ft_lr1e4/PCN_models/exp_stage2_exp1_on_lr1e4_seed42"
CFG="cfgs/PCN_models/AdaPoinTr_MSF_Pure_Group_sigmoid_feedpointrs_ft_lr1e4.yaml"
LOG_BEST="${LOG_ROOT}/test_stage2_exp1_on_lr1e4_best.log"
LOG_LAST="${LOG_ROOT}/test_stage2_exp1_on_lr1e4_last.log"
WATCH="${LOG_ROOT}/watch_stage2_exp1_on_lr1e4_tests.log"
...
if pgrep -f "main.py --test.*lr1e4" >/dev/null 2>&1; then
  echo "ERROR: lr1e4 test already running." >&2
  exit 1
fi
...
exp = Path("experiments/AdaPoinTr_MSF_Pure_Group_sigmoid_feedpointrs_ft_lr1e4/PCN_models/exp_stage2_exp1_on_lr1e4_seed42")
...
: >"$LOG_BEST"
echo "=== $(date) start best ep test (GPU${GPU_BEST}), same_epoch=${SAME_EPOCH} ===" | tee "$WATCH"
CUDA_VISIBLE_DEVICES="$GPU_BEST" nohup "$PYTHON" -u main.py --test \
  --ckpts "${EXP_DIR}/ckpt-best.pth" \
  --config "$CFG" \
  --exp_name test_stage2_exp1_on_lr1e4_best \
  --model pgst \
  --num_workers 4 \
  >>"$LOG_BEST" 2>&1 &
BEST_PID=$!
...
(
  ...
  CUDA_VISIBLE_DEVICES="$GPU_LAST" nohup "$PYTHON" -u main.py --test \
    ...
    --config "$CFG" \
    --exp_name test_stage2_exp1_on_lr1e4_last \
    --model pgst \
    --num_workers 4 \
    >>"$LOG_LAST" 2>&1 &
...
echo "watch_pid=$!"
echo "Monitor: tail -30 ${LOG_BEST} ; tail -30 ${WATCH}"
