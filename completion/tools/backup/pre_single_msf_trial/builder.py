...
def dataset_builder(args, config):
    ...
    # 优化DataLoader配置：降低SWAP压力，提升GPU利用率
    # 当num_workers=0时，不需要persistent_workers和prefetch_factor
    num_workers = int(args.num_workers)
    is_train = config.others.subset == 'train'
    
    # DataLoader参数优化
    # pin_memory=True: 加速CPU到GPU的内存拷贝
    # prefetch_factor: 预加载batch数量（只在num_workers>0时有效）
    # persistent_workers: 避免worker进程重复创建（只在num_workers>0时有效）
    dataloader_kwargs = {
    ...
    # 只有在num_workers > 0时才添加这些参数
    if num_workers > 0:
        dataloader_kwargs['prefetch_factor'] = 2  # 预加载2个batch
        dataloader_kwargs['persistent_workers'] = True  # 保持worker进程存活
...
def load_model(base_model, ckpt_path, logger = None):
    ...
    filtered_ckpt = {
        k: v for k, v in base_ckpt.items()
        if k in model_dict and v.shape == model_dict[k].shape
    }

    not_loaded_keys = [k for k in base_ckpt if k not in filtered_ckpt]
    if len(not_loaded_keys) > 0:
        print(f"Not loaded because of shape comflictions: {not_loaded_keys}")

    base_model.load_state_dict(filtered_ckpt, strict=False)
    # base_model.load_state_dict(base_ckpt)
