#!/usr/bin/env python
"""
exp3_adaptive_pooling_fixed 可视化：固定 5 个类别各 1 个样本，
展示 Partial / GT / 预测 / 骨架(coarse) 及叠加。

用法（在 completion 目录下）:
  CUDA_VISIBLE_DEVICES=0 /path/to/pgst/python visualize_exp3_fixed.py
"""
import argparse
import os
import sys
...
def main():
    ...
    parser.add_argument("--checkpoint", type=str,
                        default="./experiments/AdaPoinTr_core_new/PCN_models/exp3_adaptive_pooling_fixed/ckpt-best.pth")
    parser.add_argument("--out_dir", type=str,
                        default="./visualize_output/exp3_fixed_vis")
    ...
    with torch.no_grad():
        for _, (taxonomy_id, model_id, data) in enumerate(val_loader):
            ...
            plt.suptitle(
                f"{tax.upper()}  sample#{si}  (exp3 Fixed Adaptive Pooling, ckpt-best)",
