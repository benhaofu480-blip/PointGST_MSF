#!/usr/bin/env python
"""
exp_abs_coord_hole (Absolute Coordinate Generation + Dynamic Hole Targeting) 可视化：随机 4 个类别各 1 个样本，
展示 Partial / GT / 预测 / 骨架(coarse) 及 GT+骨架、预测+骨架 叠加。

用法（在 completion 目录下）:
  python visualize_exp_abs_coord_hole.py
  python visualize_exp_abs_coord_hole.py --seed 42 --out_dir ./visualize_output/exp_abs_coord_hole
"""
import argparse
import os
import random
...
def main():
    ...
    parser.add_argument(
        "--checkpoint",
        type=str,
        default="./experiments/pertoken_core_gpu1_ratio384/PCN_models/exp_abs_coord_hole/ckpt-best.pth",
    )
    parser.add_argument(
        "--out_dir",
        type=str,
        default="./visualize_output/exp_abs_coord_hole",
    )
    ...
    with torch.no_grad():
        for _, (taxonomy_id, model_id, data) in enumerate(val_loader):
            ...
            plot_point_cloud(
                coarse_np,
                ax4,
                f"Skeleton (coarse)\n{num_coarse} pts (384+128)",
                color="darkorange",
                s=8,
            )
            ax5 = fig.add_subplot(155, projection="3d")
            plot_overlay_dense_and_coarse(
                fine_np,
                "royalblue",
                coarse_np,
                "darkorange",
            ...
            plt.suptitle(
                f"{tax.upper()}  sample#{si}  (exp_abs_coord_hole ckpt-best)",
                fontsize=12,
                fontweight="bold",
            )
            plt.tight_layout()
            ...
            plot_overlay_dense_and_coarse(
                gt_np, "green", coarse_np, "darkorange", bx1, "GT + skeleton"
            )
            ...
            plot_overlay_dense_and_coarse(
                ...
                "darkorange",
                bx2,
                f"Prediction + skeleton (CD-L1={cd_l1:.2f})",
            )
            plt.suptitle(
                f"{tax.upper()} — GT vs Pred (both overlaid skeleton)",
                fontsize=13,
                fontweight="bold",
            )
            plt.tight_layout()
            ...
            with open(meta_path, "a", encoding="utf-8") as f:
                f.write(
                    f"{tax}\t{si}\t{model_id[0]}\tCD_L1_mm={cd_l1:.4f}\t{prefix}\n"
                )
