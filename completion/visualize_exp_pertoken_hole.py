#!/usr/bin/env python
"""
exp_pertoken_hole (Dynamic Hole Targeting) 可视化：随机 4 个类别各 1 个样本，
展示 Partial / GT / 预测 / 骨架(coarse) 及 GT+骨架、预测+骨架 叠加。

用法（在 completion 目录下）:
  python visualize_exp_pertoken_hole.py
  python visualize_exp_pertoken_hole.py --seed 42 --out_dir ./visualize_output/exp_pertoken_hole
"""
import argparse
import os
import random
...
def main():
    ...
    parser.add_argument(
        "--checkpoint",
        type=str,
        default="./experiments/pertoken_core_gpu1_ratio384/PCN_models/exp_pertoken_hole/ckpt-best.pth",
    )
    parser.add_argument(
        "--out_dir",
        type=str,
        default="./visualize_output/exp_pertoken_hole",
    )
    ...
    with torch.no_grad():
        for _, (taxonomy_id, model_id, data) in enumerate(val_loader):
            ...
            plt.suptitle(
                f"{tax.upper()}  sample#{si}  (exp_pertoken_hole ckpt-best)",
                fontsize=12,
                fontweight="bold",
            )
            plt.tight_layout()
