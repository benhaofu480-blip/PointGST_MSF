#!/usr/bin/env python
"""
exp_ratio_384_128_v2 (ckpt-best) 可视化：随机 4 个类别各 1 个样本，
展示 Partial / GT / 预测 / 骨架(coarse) 及 GT+骨架、预测+骨架 叠加。

用法（在 completion 目录下）:
  python visualize_exp_ratio384_128_v2.py
  python visualize_exp_ratio384_128_v2.py --seed 42 --out_dir ./visualize_output/exp_ratio_384_128_v2
"""
import argparse
import os
import random
...
def main():
    ...
    parser.add_argument(
        "--checkpoint",
        type=str,
        default="./experiments/pertoken_core_gpu1_ratio384/PCN_models/exp_ratio_384_128_v2/ckpt-best.pth",
    )
    parser.add_argument(
        "--out_dir",
        type=str,
        default="./visualize_output/exp_ratio_384_128_v2",
    )
    ...
    with torch.no_grad():
        for _, (taxonomy_id, model_id, data) in enumerate(val_loader):
            ...
            plt.suptitle(
                f"{tax.upper()}  sample#{si}  (exp_ratio_384_128_v2 ckpt-best)",
                fontsize=12,
                fontweight="bold",
            )
            plt.tight_layout()
